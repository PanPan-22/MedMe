    ...
    for (const s of snapshot.schedules) {
      await db.runAsync(
        ...
        [
          ...
          s.start_date, s.end_date, s.patient_id, s.updated_at,
        ],
      );
    }
    for (const l of snapshot.medication_logs) {
      await db.runAsync(
        `INSERT OR REPLACE INTO medication_logs
          (id, sync_id, medication_id, scheduled_time, log_date, timestamp, status, patient_id, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [l.id, l.sync_id, l.medication_id, l.scheduled_time, l.log_date, l.timestamp, l.status, l.patient_id, l.updated_at],
      );
    }
  });
}
